...
SizeByte                equ 1          ; Globals for different data sizes in order to make calculations with these references
SizeWord                equ 2
SizeLong                equ 4
SizeTile                equ 64
SizePalette             equ 64
...
Main:
	...
	move.l #0x18, d0  ; 3 palettes, 24 longwords, minus 1 for counter
    ...
    move.l   #SegaLogoOneFrameT, d1 ; Move number of tiles (in one animation frame) to d1
	jsr      LoadTiles              ; Jump to subroutine
	
	; Load sprite descriptors
	lea     SpriteDescs, a0		; Sprite table data
	move.w  #0x3, d0			; 3 sprites (2 normal, one animated)
	jsr     LoadSpriteTables

	; Set sprite positions
	move.w  #0x0,  d0	  ; Sprite ID
	move.w  #0xC0, d1	  ; X coord
	jsr     SetSpritePosX ; Set X pos
	move.w  #0xA0, d1	  ; Y coord
	...
	move.w  #0x2,  d0	  ; Sprite ID
	move.w  #0xC0, d1	  ; X coord
	jsr     SetSpritePosX ; Set X pos
	move.w  #0xC0, d1	  ; Y coord
	jsr     SetSpritePosY ; Set Y pos
...
GameLoop:
	...
	move.l #0x18, d0
	jsr WaitFrames   ; Wait for 24 frames to slow the anim down, just for testing

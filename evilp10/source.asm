...
RegistersClear:
    ...
    move #0x2000, sr          ; Init status register (sr AKA CPU flags: no trace, supervisor mode, all interrupt levels enabled, clear condition code bits). Without this interrupts callbacks aren't called!
...
psg_control				equ 0x00C00011 ; PSG control port (looks like doesn't have a data port, data is sent to this port)
...
; *** MEMORY MAP (GLOBALS) ***
hblank_counter		equ 0x00FF0000                ; HBlank counter at Start of RAM (longword)
vblank_counter		equ (hblank_counter+SizeLong) ; VBlank counter comes next (longword)
audio_clock			equ (vblank_counter+SizeLong) ; Clock for PSG (word)
...
; *** PSG SOUND TEST (4B0) ***
SoundTest:
    ; Generally writes 2 bytes to PSG control port with setup + data
    ; First byte of data:
	...
	move.b #%10010000, psg_control ; Set channel 0 to full volume
	
	lea chan0_notes, a0            ; Notes address to a0
	move.l #chan0_notes_count, d1  ; Number of notes to play to d1
	subi.l #0x1, d1                ; -1 for counter
	
	@NextNote:
	
	move.w (a0)+, d0       ; First byte (first part of data). Delay of note to d0 and inc. pointer (how long it will play)
	move.w (a0)+, d2       ; Second byte. Counter reset to d2 and inc. pointer (counter reset is used to generate notes, it has to do with sound generation in square waves)
	...
	move.b d3, psg_control ; Write to PSG port (first byte)
	...
	move.l d1, -(sp)       ; Backup d1 to stack pointer (first decrements then backups it). SP in a7
	jsr WaitFrames         ; Delay frames is in d0
	move.l (sp)+, d1       ; After getting back from subroutine restore d1 from stack pointer then increments it
	
	dbra d1, @NextNote
	
	move.b #%10011111, psg_control ; Silence channel 0
...
Main:
	...
	jsr SoundTest                   ; Goes to sound test
